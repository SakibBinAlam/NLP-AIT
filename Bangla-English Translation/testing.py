...
SRC_LANGUAGE,TRG_LANGUAGE = 'bn', 'en'
    ...
    def func(txt_input):
        for transform in transforms:
            if transform == token_transform[SRC_LANGUAGE]:
                txt_input = token_transform[SRC_LANGUAGE](txt_input)
            else:
...
def translation(source, variants, save_path, device):
    ...
    # src_text = text_transform[SRC_LANGUAGE](source)
    target = "It is fake target"*20
    trg_text = text_transform[TRG_LANGUAGE](target).to(device)
    # trg_text = text_transform[TRG_LANGUAGE](target)
